 ...
 * Execution: java -cp .;algs4.jar LazyPrims filename.txt
 * 
...
public class LazyPrims {
    ...
    private void lazy_prim(EdgeWeightedGraph G, int v) {
        ...
        scan(G, v);
        while (!pq.isEmpty()) {                        
            Edge e = pq.delMin();                      			// small edge on pq
            int vect = e.either(), vec = e.other(vect);        // two endpoints
            assert marked[vect] || marked[vec];
            if (marked[vect] && marked[vec]){
				continue;
			}				
    ...
    private void scan(EdgeWeightedGraph G, int v) {
        ...
        // add incident edges to v onto PQ
       assert !marked[v];
        marked[v] = true;
        for (Edge e : G.adj(v))
